...
const typesOptions = [
  { label: "Select Number Of Bedrooms ", value: " " },
  { label: "1 ", value: 1 },
  { label: "2 ", value: 2 },
  { label: "3 ", value: 3 },
  { label: "4 ", value: 4 },
...
const SearchBar = ({ allCommunities }) => {
  ...
  const [comunityValue, setComunityValue] = useState(1);
  const [numberOfBedrooms, setNumberOfBedrooms] = useState(1);
  const [price, setPrice] = useState(1);
  const Communities = [{ label: "Select Community", value: "" }];
  const [data, setData] = useState();
  useEffect(() => {
    allCommunities.map((comunity) =>
      Communities.push({ label: comunity.name, value: comunity.id })
    );
  }, [allCommunities]);

  const handleChangeComunityValue = (e) => {
    setComunityValue(e.value);
  };
  const handleChangeNumberOfBedrooms = (e) => {
    setNumberOfBedrooms(e.value);
  };
            ...
            <div className="sele">
              ...
              <div className="selecty">
                ...
                <Select
                  ...
                  onChange={handleChangeComunityValue}
                />
              </div>
            </div>
            <div className="sele">
